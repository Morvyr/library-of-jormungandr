...
class CSVCleaner(BaseTool):
    ...
    def process(self, filepath):
        ...
        try:
            ...
            self.logger.info(f"DEBUG: After _standardize_text, Customer_Name[0] = '{df['Customer_Name'].iloc[0]}'")
            self.logger.info(f"DEBUG: After _standardize_text, Email[0] = '{df['Email'].iloc[0]}'")

            self.logger.info(f"DEBUG: Column order before saving: {list(df.columns)}")
    ...
    def _standardize_text(self, df):
        ...
        self.logger.info(f"DEBUG START: First 3 Customer_Name = {df['Customer_Name'].head(3).tolist()}")
        self.logger.info(f"DEBUG START: First 3 Email = {df['Email'].head(3).tolist()}")
        ...
        for col in name_columns:
            if df[col].dtype == 'object': 
                self.logger.info(f"Processing name column: {col}")
        ...
        for col in email_columns:
            if df[col].dtype == 'object':
                self.logger.info(f"Processing email column: {col}")
                before = df[col].iloc[0] if len(df) > 0 else None
        ...
        for col in status_columns:
            if df[col].dtype == 'object':
                self.logger.info(f"Processing status column: {col}")
                before = df[col].iloc[0] if len(df) > 0 else None
                df[col] = df[col].apply(lambda x: x.title() if pd.notna(x) and isinstance(x, str) else x)
                after = df[col].iloc[0] if len(df) > 0 else None
                self.logger.info(f"  Before: '{before}' -> After: '{after}'")
